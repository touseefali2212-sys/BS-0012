...
export async function registerRoutes(
  ...
  app.post("/api/reseller-wallet/recharge", requireAuth, async (req, res) => {
    try {
      ...
      const parsed = schema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ message: "Invalid data", errors: parsed.error.flatten() });
      const reseller = await storage.rechargeResellerWallet(parsed.data.resellerId, parsed.data.amount, parsed.data.reference, parsed.data.paymentMethod, parsed.data.remarks, "admin", parsed.data.paymentStatus, parsed.data.vendorId, parsed.data.bankAccountId, parsed.data.paidAmount, parsed.data.senderName);
      res.json(reseller);
    } catch (error: any) { res.status(400).json({ message: error.message }); }
  });
