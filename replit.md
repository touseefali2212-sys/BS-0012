...
- **Core Modules**:
    ...
    - **Package Change & Bandwidth Management**: Full workflow page at `/package-change` with 7 tabs (New Request, Pending Approval, Approved, Implementation Queue, Completed, Rejected, History). Supports all customer types (Normal, CIR, Corporate) with customer search, billing preview calculator, approve/reject/implement dialogs, and audit trail. Schema: `packageChangeRequests` table. API: GET/POST/PATCH/DELETE `/api/package-change-requests`. Accessible from sidebar under Customers and via "Package Change Request" button on all customer profile pages.
