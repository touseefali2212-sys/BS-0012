...
import {
  ...
  ArrowUpDown, Settings, Bell, Smartphone, Zap, Link2, Unlink, Radio,
} from "lucide-react";
...
function CorpEditDialog({ open, onClose, customer, id }: { open: boolean; onClose: () => void; customer: CorporateCustomer; id: string }) {
  ...
  const sections = CORP_FORM_SECTIONS;
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Edit Corporate Customer</DialogTitle></DialogHeader>
        <div className="flex gap-1 flex-wrap mb-4">
          {sections.map((s, i) => (
            <Button key={s} variant={section === i ? "default" : "outline"} size="sm" onClick={() => setSection(i)}
              className={section === i ? "bg-gradient-to-r from-[#002B5B] to-[#005EFF] text-white text-xs" : "text-xs"} data-testid={`btn-corp-section-${i}`}>
              {s}
            </Button>
          ))}
        </div>
        <Form {...form}>
          <form onSubmit={form.handleSubmit((d) => updateMutation.mutate(d))} className="space-y-4">
            <CorporateCustomerFormFields form={form} section={section} />
            <div className="mt-4 pt-4 border-t">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex gap-2">
                  {section > 0 && <Button type="button" variant="outline" size="sm" onClick={() => setSection(s => s - 1)}>Previous</Button>}
                  {section < 4 && <Button type="button" variant="outline" size="sm" onClick={() => setSection(s => s + 1)}>Next</Button>}
                  <span className="text-xs text-muted-foreground self-center">Step {section + 1} of 5</span>
                </div>
                <div className="flex gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={onClose}>Cancel</Button>
                  <Button type="submit" size="sm" disabled={updateMutation.isPending} className="bg-gradient-to-r from-[#002B5B] to-[#005EFF] text-white" data-testid="button-corp-edit-submit">
...
export default function CorporateCustomerProfilePage() {
  ...
  const { data: ontIntegration, isLoading: ontLoading } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/customers/${id}/ont-integration?customerType=corporate`, { credentials: "include" });
      if (!res.ok) return [];
      return res.json();
    },
    enabled: !!id,
  ...
  const assignOnuMutation = useMutation({
    mutationFn: async (onuDeviceId: string) => {
      const res = await apiRequest("POST", `/api/customers/${id}/ont-integration`, { onuDeviceId, customerType: "corporate" });
      return res.json();
    },
    onSuccess: () => {
  ...
  const unlinkOnuMutation = useMutation({
    mutationFn: async (onuId: number) => {
      const res = await apiRequest("DELETE", `/api/customers/${id}/ont-integration/${onuId}?customerType=corporate`);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/customers", id, "ont-integration"] });
