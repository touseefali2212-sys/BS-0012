...
function ProfilePhotoUpload({ value, onChange }: { value: string; onChange: (url: string) => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <p className="text-xs text-muted-foreground">{uploading ? "Uploading..." : "Click to update photo"}</p>
    </div>
  );
}
...
export default function EditResellerPage() {
  ...
  const [, params] = useRoute("/resellers/:id/edit");
  const resellerId = params?.id ? parseInt(params.id) : null;
  ...
  useEffect(() => {
    if (!reseller || prefilled || !branches) return;
    const r = reseller;

    const matchedBranch = branches.find(b => b.name === r.branch);
    const branchId = matchedBranch ? String(matchedBranch.id) : "";

    setForm({
      name: r.name || "",
      companyName: r.companyName || "",
      contactName: r.contactName || "",
      resellerType: r.resellerType || "authorized_dealer",
      gender: r.gender || "",
      ...
      maxCustomerLimit: String(r.maxCustomerLimit || 0),
      notes: r.notes || "",
      ...
      uplinkType: r.uplinkType || "",
      uplink: r.uplink || "",
      exchangeTowerPopName: r.exchangeTowerPopName || "",
      portId: r.portId || "",
      vlanId: r.vlanId || "",
      media: r.media || "",
      connectionType: r.connectionType || "",
      vlanInput: "",
      bandwidthPlan: r.bandwidthPlan || "",
      ipAssignment: r.ipAssignment || "dynamic",
      nasId: r.nasId || "",
      serviceZone: r.serviceZone || "",
      vlanIdAllowed: r.vlanIdAllowed ?? false,
      vlanIdNote: r.vlanIdNote || "",
      vendorPanelAllowed: r.vendorPanelAllowed ?? false,
      ...
      agreementEndDate: r.agreementEndDate || "",
      autoRenewal: r.autoRenewal ?? false,
      cnicPicture: r.cnicPicture || "",
      cnicBackPicture: r.cnicBackPicture || "",
      officePicture: r.officePicture || "",
      agreementFile: r.agreementFile || "",
    });

    if (r.vlanId) setVlanIds(r.vlanId.split(",").map((s: string) => s.trim()).filter(Boolean));

    if (r.assignedVendorPanels) {
      try {
        const panels = JSON.parse(r.assignedVendorPanels);
        if (Array.isArray(panels)) setAddedVendorPanels(panels);
      } catch {}
    }

    setPrefilled(true);
  }, [reseller, branches, prefilled]);

  useEffect(() => {
    if (!reseller || !packagesList || !prefilled) return;
    if (!reseller.assignedPackages) return;
    const ids = reseller.assignedPackages.split(",").map((s: string) => parseInt(s.trim())).filter(Boolean);
    const pkgs = ids.map((id: number) => {
      ...
      return {
        ...
        speed: pkg.speed || "",
        vendorId: pkg.vendorId ? String(pkg.vendorId) : "",
        vendorName: "",
    ...
    if (pkgs.length > 0) setAddedPackages(pkgs as any);
  }, [reseller, packagesList, prefilled]);
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        mapLatitude: form.mapLatitude || null, mapLongitude: form.mapLongitude || null,
        ...
        assignedPackages: addedPackages.length > 0 ? addedPackages.map(p => p.packageId).join(",") : null,
      ...
      const res = await apiRequest("PATCH", `/api/resellers/${resellerId}`, payload);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/resellers"] });
      queryClient.invalidateQueries({ queryKey: ["/api/resellers", resellerId] });
      toast({ title: "Changes saved", description: `${form.name} profile has been updated.` });
  ...
  const filteredAreas = areas?.filter(a => !selectedBranch || a.branch === selectedBranch.name);

  const handleBranchChange = (branchId: string) => {
    const branch = branches?.find(b => String(b.id) === branchId);
    setForm(prev => ({ ...prev, branch: branchId, area: "", city: branch?.city || prev.city }));
  };
  const handleAreaChange = (areaName: string) => setForm(prev => ({ ...prev, area: areaName }));
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-muted-foreground text-sm">Loading reseller profile...</p>
        </div>
      </div>
    );
  }

  if (!reseller && !resellerLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center space-y-3">
          <p className="text-lg font-semibold">Reseller not found</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 dark:from-slate-950 dark:via-blue-950/10 dark:to-indigo-950/10">
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => setLocation("/resellers")} className="gap-1.5 text-muted-foreground hover:text-foreground" data-testid="button-back-resellers">
            <ChevronLeft className="h-4 w-4" /> Back to Resellers
          </Button>
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-foreground">Edit Reseller</h1>
            <p className="text-sm text-muted-foreground">{reseller?.name} — update information and settings</p>
          </div>
          <div className="flex items-center gap-2">
            {totalMissingRequired > 0 && (
              <Badge variant="outline" className="border-amber-300 bg-amber-50 text-amber-700 dark:bg-amber-950/20 gap-1">
                <AlertCircle className="h-3 w-3" />
                {totalMissingRequired} required {totalMissingRequired === 1 ? "field" : "fields"} missing
              </Badge>
            )}
            <Button variant="outline" size="sm" onClick={() => { setPrefilled(false); }} className="gap-1.5" data-testid="button-reset">
        ...
        {/* Tab navigation */}
        <div className="flex gap-1 overflow-x-auto bg-white dark:bg-slate-900 border rounded-xl p-1 shadow-sm">
          {tabItems.map((tab, i) => {
            const Icon = tab.icon;
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap flex-1 justify-center
                  ${isActive ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-md" :
                    isComplete ? "text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-950/20" :
                    "text-muted-foreground hover:bg-muted/60"}`}
              >
                {isComplete && !isActive ? <CheckCircle2 className="h-3.5 w-3.5" /> : <Icon className="h-3.5 w-3.5" />}
        ...
        <div className="space-y-5 pb-28">
          ...
          {activeTab === "basic" && (
            <div className="space-y-5">
              <Card>
                <SectionHeader icon={Handshake} title="Personal Info" description="Reseller identity, contact and location" />
                <CardContent className="space-y-5">
                  {/* Profile Photo */}
                  <div className="flex justify-center">
                    <ProfilePhotoUpload value={form.profilePicture} onChange={url => update("profilePicture", url)} />
                  </div>
                  <Separator />
                  ...
                  <FieldGroup>
                    <Field label="Company Name">
                      <Input value={form.companyName} onChange={e => update("companyName", e.target.value)} placeholder="e.g. NetLink Solutions" data-testid="input-company-name" />
                    </Field>
                    <Field label="Reseller Name" required error={errors.name}>
                      <Input value={form.name} onChange={e => update("name", e.target.value)} placeholder="Full name" data-testid="input-reseller-name" className={errors.name ? "border-red-500" : ""} />
                    </Field>
                  </FieldGroup>
                  ...
                  <FieldGroup>
                    ...
                    <Field label="Mobile No" required error={errors.phone}>
                      <Input value={form.phone} onChange={e => update("phone", e.target.value)} placeholder="03XX-XXXXXXX" data-testid="input-phone" className={errors.phone ? "border-red-500" : ""} />
                    </Field>
                  </FieldGroup>
                  ...
                  <FieldGroup>
                    <Field label="Phone No">
                      <Input value={form.secondaryPhone} onChange={e => update("secondaryPhone", e.target.value)} placeholder="Landline or secondary" data-testid="input-secondary-phone" />
                    </Field>
                    <Field label="Email" required error={errors.email}>
                      <Input type="email" value={form.email} onChange={e => update("email", e.target.value)} placeholder="email@domain.com" data-testid="input-email" className={errors.email ? "border-red-500" : ""} />
                    </Field>
                  </FieldGroup>
                  ...
                  <FieldGroup>
                    <Field label="CNIC / NID" required error={errors.cnic}>
                      <Input value={form.cnic} onChange={e => update("cnic", e.target.value)} placeholder="XXXXX-XXXXXXX-X" data-testid="input-cnic" className={errors.cnic ? "border-red-500" : ""} />
                    </Field>
                    <Field label="Branch" required error={errors.branch}>
                      <Select value={form.branch} onValueChange={handleBranchChange} data-testid="select-branch">
                  ...
                  <FieldGroup>
                    <Field label="Area" required error={errors.area}>
                      <Select value={form.area} onValueChange={handleAreaChange} disabled={!form.branch} data-testid="select-area">
                    ...
                    <Field label="City" required error={errors.city}>
                      <Input value={form.city} onChange={e => update("city", e.target.value)} placeholder="Auto-filled from branch" data-testid="input-city" className={`${errors.city ? "border-red-500" : ""} ${selectedBranch?.city ? "bg-muted/50" : ""}`} readOnly={!!selectedBranch?.city} />
                    </Field>
                  </FieldGroup>

                  {/* Address */}
                  <Field label="Address" required error={errors.address}>
                    <Textarea value={form.address} onChange={e => update("address", e.target.value)} placeholder="Street, block, building, landmark..." className={`min-h-[80px] ${errors.address ? "border-red-500" : ""}`} data-testid="input-address" />
                  </Field>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={openMapPicker} data-testid="button-get-gps" className="gap-1.5 border-blue-300 text-blue-700 hover:bg-blue-50">
                        <Map className="h-3.5 w-3.5" /> Get GPS Location from MAP
                      </Button>
                    </div>
                    <FieldGroup>
                    ...
                    {form.mapLatitude && form.mapLongitude && (
                      <div className="rounded-md overflow-hidden border h-48">
                        <MapContainer center={[parseFloat(form.mapLatitude) || 31.5204, parseFloat(form.mapLongitude) || 74.3587]} zoom={15} style={{ height: "100%", width: "100%" }} zoomControl={false} dragging={false} scrollWheelZoom={false} doubleClickZoom={false} attributionControl={false}>
                  ...
                  <Field label="NOTE / Remarks">
                    <Textarea value={form.notes} onChange={e => update("notes", e.target.value)} placeholder="Internal notes or remarks about this reseller..." className="min-h-[80px]" data-testid="input-notes" />
                  </Field>

                  <Separator />

                  {/* Reseller Type & Status */}
                  <FieldGroup cols={3}>
                    <Field label="Reseller Type">
                      <Select value={form.resellerType} onValueChange={v => update("resellerType", v)}>
                        ...
                        <SelectContent>
                          {(resellerTypesList || []).map(t => <SelectItem key={t.key} value={t.key}>{t.label}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </Field>
                    <Field label="Status">
                  ...
                  <FieldGroup>
                    <Field label="NTN Number">
                      <Input value={form.ntn} onChange={e => update("ntn", e.target.value)} placeholder="NTN number" data-testid="input-ntn" />
                    </Field>
                    <Field label="Registration Form No">
                      <Input value={form.registrationFormNo} onChange={e => update("registrationFormNo", e.target.value)} placeholder="Registration number" data-testid="input-reg-form-no" />
                    </Field>
                  </FieldGroup>
                  ...
                  <FieldGroup>
                    ...
                    <Field label="Max Customer Limit">
                      <Input type="number" value={form.maxCustomerLimit} onChange={e => update("maxCustomerLimit", e.target.value)} placeholder="0 = Unlimited" data-testid="input-max-customers" />
                    </Field>
                  </FieldGroup>
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2 pt-1">
                    ...
                    <div className="flex gap-2">
                      <Input value={form.vlanInput} onChange={e => update("vlanInput", e.target.value)} onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); addVlanId(); } }} placeholder="Enter VLAN ID and press Add" className="flex-1" data-testid="input-vlan-id" />
                      <Button type="button" variant="outline" size="sm" onClick={addVlanId} disabled={!form.vlanInput.trim()} data-testid="button-add-vlan" className="gap-1.5 shrink-0">
                    ...
                    {vlanIds.length > 0 && (
                      <div className="flex flex-wrap gap-2 pt-1">
                        {vlanIds.map(id => (
                          <div key={id} className="flex items-center gap-1.5 bg-blue-50 border border-blue-200 text-blue-800 text-xs font-medium px-2.5 py-1 rounded-full" data-testid={`badge-vlan-${id}`}>
                            <span>VLAN {id}</span>
                            <button type="button" onClick={() => removeVlanId(id)} className="text-blue-500 hover:text-red-500 transition-colors" data-testid={`remove-vlan-${id}`}>
                              <X className="h-3 w-3" />
                            </button>
                          </div>
                        ))}
                    ...
                    {vlanIds.length === 0 && <p className="text-xs text-muted-foreground">No VLAN IDs added yet. You can add multiple.</p>}
                  </div>
                  ...
                  <FieldGroup cols={3}>
                    <Field label="Bandwidth Plan">
                      <Input value={form.bandwidthPlan} onChange={e => update("bandwidthPlan", e.target.value)} placeholder="e.g. 100Mbps" data-testid="input-bandwidth-plan" />
                    </Field>
                    ...
                    <Field label="NAS ID">
                      <Input value={form.nasId} onChange={e => update("nasId", e.target.value)} placeholder="NAS identifier" data-testid="input-nas-id" />
                    </Field>
                  </FieldGroup>
                  <Field label="Service Zone">
                    <Input value={form.serviceZone} onChange={e => update("serviceZone", e.target.value)} placeholder="e.g. Zone A, North Sector" data-testid="input-service-zone" />
                  </Field>
                </CardContent>
              </Card>
            </div>
                ...
                <CardContent className="space-y-4">
                  <FieldGroup>
                      ...
                      <Select value={pkgForm.vendorId} onValueChange={v => setPkgForm(prev => ({ ...prev, vendorId: v, packageId: "", vendorPrice: "0" }))}>
                        ...
                        <SelectContent>
                          {(vendors || []).filter(v => v.panelUrl).length > 0
                            ? (vendors || []).filter(v => v.panelUrl).map(v => <SelectItem key={v.id} value={String(v.id)}>{v.name}</SelectItem>)
                    ...
                    <Field label="Select Package" required>
                      <Select value={pkgForm.packageId} disabled={!pkgForm.vendorId} onValueChange={v => {
                        ...
                        setPkgForm(prev => ({ ...prev, packageId: v, vendorPrice: pkg?.price || "0" }));
                      }}>
                        ...
                        <SelectContent>
                          {(packagesList || []).filter(p => !pkgForm.vendorId || String(p.vendorId) === pkgForm.vendorId || p.vendorId == null).map(p => (
                            <SelectItem key={p.id} value={String(p.id)}>{p.name}{p.speed ? ` — ${p.speed}` : ""}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </Field>
                  ...
                  <FieldGroup cols={3}>
                    <Field label="Vendor Price (Rs.)">
                      <Input value={pkgForm.vendorPrice} readOnly className="bg-muted/50 font-medium" data-testid="input-vendor-price" />
                    </Field>
                    <Field label="Profit (Rs.)">
                      <Input type="number" value={pkgForm.profit} onChange={e => setPkgForm(prev => ({ ...prev, profit: e.target.value }))} placeholder="0" data-testid="input-profit" />
                    </Field>
                    <Field label="Reseller Price (Rs.)">
                      <div className={`flex items-center h-10 px-3 rounded-md border text-sm font-semibold ${parseFloat(pkgForm.profit) > 0 ? "bg-green-50 border-green-200 text-green-700" : "bg-muted/50"}`}>
                        Rs. {pkgResellerPrice}
                      </div>
                    </Field>
                  </FieldGroup>
                  <Button type="button" variant="outline" size="sm" onClick={handleAddPackage} disabled={!pkgForm.packageId || !pkgForm.vendorId} data-testid="button-add-package" className="gap-1.5">
                    ...
                    <div className="border rounded-lg overflow-hidden mt-2">
                      ...
                      {addedPackages.map((p, i) => (
                        <div key={i} className="px-3 py-2.5 border-t text-sm grid grid-cols-7 gap-2 items-center">
                          ...
                          <Button type="button" variant="ghost" size="sm" onClick={() => setAddedPackages(prev => prev.filter((_, j) => j !== i))} data-testid={`button-remove-package-${i}`}>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Enable reseller to access external vendor portals</p>
                    </div>
                    <Switch checked={form.vendorPanelAllowed} onCheckedChange={v => update("vendorPanelAllowed", v)} data-testid="switch-vendor-panel-allowed" />
                  </div>
                    ...
                    <>
                      ...
                      <FieldGroup>
                        ...
                        <Field label="Panel URL">
                          <Input value={vpForm.panelUrl} onChange={e => setVpForm(prev => ({ ...prev, panelUrl: e.target.value }))} placeholder="https://panel.vendor.com" data-testid="input-vp-url" />
                        </Field>
                      </FieldGroup>
                      <FieldGroup>
                        <Field label="Panel Username">
                          <Input value={vpForm.panelUsername} onChange={e => setVpForm(prev => ({ ...prev, panelUsername: e.target.value }))} placeholder="Username" data-testid="input-vp-username" />
                        </Field>
                        <Field label="Panel Password">
                          <div className="relative">
                            <Input type={showVpPassword ? "text" : "password"} value={vpForm.panelPassword} onChange={e => setVpForm(prev => ({ ...prev, panelPassword: e.target.value }))} placeholder="Password" data-testid="input-vp-password" className="pr-10" />
                            <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1 h-7 w-7" onClick={() => setShowVpPassword(p => !p)}>
                              {showVpPassword ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                            </Button>
                          </div>
                        </Field>
                      </FieldGroup>
                      <Button type="button" variant="outline" size="sm" onClick={handleAddVendorPanel} disabled={!vpForm.vendorId || !vpForm.panelUrl} data-testid="button-add-vendor-panel" className="gap-1.5">
                      ...
                      {addedVendorPanels.length > 0 && (
                        <div className="border rounded-lg overflow-hidden mt-2">
                          <div className="px-3 py-2 bg-[#1a3a5c] text-white text-xs font-semibold uppercase tracking-wide grid grid-cols-4 gap-2">
                            <span>Vendor</span><span>Panel URL</span><span>Username</span><span>Action</span>
                          </div>
                          {addedVendorPanels.map((p, i) => (
                            <div key={i} className="px-3 py-2.5 border-t text-sm grid grid-cols-4 gap-2 items-center">
                              <span className="font-medium">{p.vendorName || p.vendorId}</span>
                              <a href={p.panelUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline truncate text-xs">{p.panelUrl}</a>
                              <span className="text-muted-foreground">{p.panelUsername || "—"}</span>
                              <Button type="button" variant="ghost" size="sm" onClick={() => setAddedVendorPanels(prev => prev.filter((_, j) => j !== i))} data-testid={`button-remove-panel-${i}`}>
                                <Trash2 className="h-4 w-4 text-red-500" />
                              </Button>
                            </div>
                          ))}
          ...
          {activeTab === "billing" && (
            <div className="space-y-5">
              <Card>
                <SectionHeader icon={Wallet} title="Wallet & Balance" description="Financial limits and billing configuration" />
                <CardContent className="space-y-4">
                  <FieldGroup cols={3}>
                    <Field label="Wallet Balance (Rs.)">
                      <Input type="number" value={form.walletBalance} onChange={e => update("walletBalance", e.target.value)} placeholder="0.00" data-testid="input-wallet-balance" />
                    </Field>
                    <Field label="Credit Limit (Rs.)">
                      <Input type="number" value={form.creditLimit} onChange={e => update("creditLimit", e.target.value)} placeholder="0.00" data-testid="input-credit-limit" />
                    </Field>
                    <Field label="Security Deposit (Rs.)">
                      <Input type="number" value={form.securityDeposit} onChange={e => update("securityDeposit", e.target.value)} placeholder="0.00" data-testid="input-security-deposit" />
                    </Field>
                  </FieldGroup>
                  <FieldGroup>
                    <Field label="Opening Balance (Rs.)">
                      <Input type="number" value={form.openingBalance} onChange={e => update("openingBalance", e.target.value)} placeholder="0.00" data-testid="input-opening-balance" />
                    </Field>
              ...
              <Card>
                <SectionHeader icon={Briefcase} title="Bank Details" description="Bank account for payments" />
                <CardContent className="space-y-4">
                  <FieldGroup>
                    <Field label="Bank Name">
                      <Input value={form.bankName} onChange={e => update("bankName", e.target.value)} placeholder="e.g. HBL, MCB" data-testid="input-bank-name" />
                    </Field>
                    <Field label="Account Title">
                      <Input value={form.bankAccountTitle} onChange={e => update("bankAccountTitle", e.target.value)} placeholder="Account holder name" data-testid="input-bank-account-title" />
                    </Field>
                  </FieldGroup>
                  <FieldGroup>
                    <Field label="Account Number">
                      <Input value={form.bankAccountNumber} onChange={e => update("bankAccountNumber", e.target.value)} placeholder="IBAN or account number" data-testid="input-bank-account-number" />
                    </Field>
                    <Field label="Branch Code">
                      <Input value={form.bankBranchCode} onChange={e => update("bankBranchCode", e.target.value)} placeholder="Branch code" data-testid="input-bank-branch-code" />
                    </Field>
                  </FieldGroup>
                </CardContent>
              </Card>
          ...
          {activeTab === "agreement" && (
            <div className="space-y-5">
              <Card>
                <SectionHeader icon={ScrollText} title="Agreement Details" description="Reseller agreement type and validity period" />
                <CardContent className="space-y-4">
                  <FieldGroup>
                    ...
                    <div className="flex items-center justify-between rounded-lg border p-3 h-fit mt-auto">
                      <div>
                        <p className="text-sm font-medium">Auto Renewal</p>
                        <p className="text-xs text-muted-foreground">Renew agreement automatically</p>
                      </div>
                      <Switch checked={form.autoRenewal} onCheckedChange={v => update("autoRenewal", v)} data-testid="switch-auto-renewal" />
                    </div>
                  </FieldGroup>
                  <FieldGroup>
              ...
              <Card>
                <SectionHeader icon={Upload} title="Upload Agreement" description="Signed agreement document (PDF or image)" />
                <CardContent className="space-y-3">
                  <FileUploadPreview label="Agreement Document" value={form.agreementFile} onChange={url => update("agreementFile", url)} testId="upload-agreement-file" />
                  {form.agreementFile && (
                    <div className="flex items-center gap-2 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 px-3 py-2">
                      ...
                      <span className="text-sm text-green-700 dark:text-green-400 flex-1 truncate">Agreement uploaded successfully</span>
                      <a href={form.agreementFile} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline font-medium shrink-0">View</a>
                    </div>
                  )}
                </CardContent>
              </Card>
          ...
          {activeTab === "documents" && (
            <div className="space-y-5">
              <Card>
                <SectionHeader icon={Image} title="KYC Documents" description="Identity verification documents" />
                <CardContent className="space-y-4">
                  <FieldGroup>
                    <FileUploadPreview label="CNIC Front" required value={form.cnicPicture} onChange={url => update("cnicPicture", url)} testId="upload-cnic-front" />
                    <FileUploadPreview label="CNIC Back" required value={form.cnicBackPicture} onChange={url => update("cnicBackPicture", url)} testId="upload-cnic-back" />
                  </FieldGroup>
                  <FileUploadPreview label="Office Picture" value={form.officePicture} onChange={url => update("officePicture", url)} testId="upload-office-picture" />
                </CardContent>
              </Card>
            </div>
          )}
        ...
        {/* Sticky bottom bar */}
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border-t shadow-lg">
          <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setLocation("/resellers")} className="gap-2" data-testid="button-cancel">
      ...
      <DialogContent className="max-w-2xl p-0 overflow-hidden">
        <DialogHeader className="px-5 pt-5 pb-3">
          <DialogTitle className="flex items-center gap-2 text-base">
            <Map className="h-4 w-4 text-blue-600" /> Pick Location on Map
          </DialogTitle>
        ...
        <div className="h-[420px] w-full">
          <MapContainer center={mapPickedPos ? [mapPickedPos.lat, mapPickedPos.lng] : [30.3753, 69.3451]} zoom={mapPickedPos ? 15 : 5} style={{ height: "100%", width: "100%" }}>
            ...
            {mapPickedPos && <Marker position={[mapPickedPos.lat, mapPickedPos.lng]} icon={pickerIcon} />}
          </MapContainer>
        </div>
        <DialogFooter className="px-5 py-3 bg-muted/30 border-t flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" disabled={!mapPickedPos} onClick={confirmMapLocation} className="bg-blue-600 hover:bg-blue-700 text-white">
