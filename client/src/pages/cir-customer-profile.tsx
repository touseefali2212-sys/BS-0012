...
import {
  ...
  ArrowUpDown, Settings, Smartphone, Zap, Link2, Unlink, Radio,
} from "lucide-react";
...
export default function CirCustomerProfilePage() {
  ...
  const { data: ontIntegration, isLoading: ontLoading } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/customers/${id}/ont-integration?customerType=cir`, { credentials: "include" });
      if (!res.ok) return [];
      return res.json();
    },
    enabled: !!id,
  ...
  const assignOnuMutation = useMutation({
    mutationFn: async (onuDeviceId: string) => {
      const res = await apiRequest("POST", `/api/customers/${id}/ont-integration`, { onuDeviceId, customerType: "cir" });
      return res.json();
    },
    onSuccess: () => {
  ...
  const unlinkOnuMutation = useMutation({
    mutationFn: async (onuId: number) => {
      const res = await apiRequest("DELETE", `/api/customers/${id}/ont-integration/${onuId}?customerType=cir`);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/customers", id, "ont-integration"] });
        ...
        <div className="w-[280px] shrink-0 border-r bg-gradient-to-b from-[#1a2332] to-[#243447] text-white">
          ...
          <div className="px-4 pt-4 pb-2 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <Button size="sm" variant="secondary" className="text-[10px] h-8 gap-1" onClick={() => setLocation(`/add-customer?type=cir&edit=${id}`)}><Edit className="h-3 w-3" /> Edit Profile</Button>
